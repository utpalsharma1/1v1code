...
#### The divergence marker — three definitions, and the one we will build

**"Where the match was decided" is not an event in the log, it is an inference over it, and the candidate inferences are not equivalent.** Written down because the marker is the most interesting thing on the timeline and therefore the one most likely to get an authoritative-looking tick over a definition nobody chose.

| definition | what it needs | what it claims |
| --- | --- | --- |
| **where the lead last changed hands** | the log, and nothing else | a fact: after this point one player was ahead and stayed ahead |
| the last moment the trailing player could still have won | a model of remaining time against typing rate | a counterfactual |
| where the two documents diverged into correct and not | running the judge over intermediate editor states | a claim about code that was never submitted |

**Default: where the lead last changed hands.** It is computable from `submission.verdict` events alone, it needs nothing the log does not already contain, and it is honest about what it asserts — it marks the last reversal, not a prediction. The other two are strictly more interesting and strictly more expensive, and both would put a confident marker over a guess. Revisit them when there is a reason; do not ship one because it sounds better.

**Not built as of Phase 3B.** The other markers — submissions, verdicts, drops, idle pauses over 20s — are, and they carry labels naming the event and the side.
...
**Every sample is generated by our own code, and that is a licence decision, not an aesthetic one.** This repository is public and AGPL. A sample with unclear rights in it is a real problem rather than a footnote, and "we found it on a free sounds site" is not a licence — most such sites carry per-asset terms that differ from the site's banner. So the cues are synthesised: `packages/ui/src/lib/sound-design.ts` renders each one from oscillators and seeded noise, and there is no `assets/sfx` directory. **Do not add an audio file to this repository without recording its licence next to it**, and prefer extending the synthesis.

**The synthesis is pure DSP over `Float32Array`, deliberately, so that §13.7 applies to it.** A Web Audio node graph can only be exercised in a browser, which in practice means it is never asserted on at all — "the sound code did not throw" is exactly a check that cannot fail. Pure functions are testable in Node, and `sound-design.test.ts` measures real properties: length, peak, RMS, dominant frequency, seam continuity on the loop, and the cues summed against each other.

**Cues are checked against each other, not only individually.** §6.6 fires a test tick every `dur.reveal` while §6.5's clutch loop runs underneath and a compile pulse can land on top — three at once is the normal case in a close match. A cue that is perfect alone and inaudible in context is the single-layer version of the same mistake.

**Loudness is compared over a short window, never over the whole buffer.** Whole-buffer RMS divides by total length, so a 2.4s sting with a reverb tail scores below a 0.9s kick — which produced a confidently wrong result the first time it was asserted. Use the loudest ~400ms window, which is roughly the ear's integration time.
